...
- **계산기 원칙(사용자 지정, 2026-09-23)**: Claude Code·Codex·Antigravity(지휘자)에게 Ollama는 비용 0원의 전자계산기다. 지휘자는 코드를 손으로 쓰지 않고 지시문·숨은 인수·diff 판정만 한다. 구현은 `pilot run --worker auto`로 맡긴다(구체적 지시 → Ollama 먼저, REWORK·로컬 실패 시 Antigravity / 모호한 지시 → Antigravity). Antigravity도 예산이 드는 하인이므로 단순 노동(요약·변환·반복 편집·테스트 틀·로그 분류)은 `pilot run --worker local` 또는 `olla ask -f <파일>`로 Ollama에 먼저 넘긴다. 커밋 관문 `.githooks/commit-msg`(`v7_harness/calculator_gate.py`)가 APPLIED pilot 결과가 아닌 `v7_harness/*.py` 변경을 막고, 예외는 커밋 메시지 `Calculator-Exempt: <이유>` 줄로만 남긴다(`docs/01` §16).
- **올라마 = '생각 없는 전자계산기이자 3대 도구의 토큰절약도구' 영구 고정(사용자 지정, 2026-09-23, 2026-09-24 재확인)**: 올라마는 스스로 생각하거나 계획할 수 없는 생각 없는 단순 도구인 전자계산기이자, 3대 도구의 유료 API 토큰 예산을 보존하기 위한 비용 0원의 단순 노동 일꾼이다. 지휘자는 사용자의 구상 프로세스에 따라 구체성 80점 이상의 세밀한 작업명령서(`docs/24`)를 작성해 넘겨야 하며, 올라마에게 주관적 추론이나 설계를 요구할 수 없다. 4대 불변식: (1) **판정권 배제**: 일꾼은 스스로 성공/실패를 판정할 수 없으며 결정론적 인수 테스트(Acceptance Gate)만이 판정한다. (2) **단순 반복 노동 우선 위임**: 요약, 줄 구조 지도(`local_read_map`), 변환, 형식 맞추기, 테스트 스텁 생성은 유료 토큰 사용 전 반드시 일꾼에 먼저 넘긴다. (3) **구체성 격리**: 프롬프트 구체성 80점 이상(대상 파일 1개, SEARCH/REPLACE 명시)일 때만 위임한다. (4) **3대 도구 공통 의무**: Codex, Claude Code, Antigravity 3대 도구 모두가 동일하게 일꾼을 부려 계정 한도를 방어하며, 일꾼을 부리지 않고 유료 토큰을 낭비하는 것은 설계 결함으로 판정한다.
