...
- **Claude Code는 Codex와 독립 분리된, 동등한 부지휘자다(사용자 지정, 2026-09-21 / 독립 분리 2026-09-23).** Codex의 지휘 계통에 속하지 않으며 Codex의 지시를 받지 않는다. Codex 활동 중에는 자기 작업과 독립 검증을 스스로 계획·수행하되, 같은 단계·파일 범위·브랜치·작업트리·외부 자원을 Codex와 동시에 수정하지 않고 판정이 다르면 증거를 붙여 이견을 기록한다. Codex 부재 중(한도·정지·무응답)에는 Codex 대신 지휘하며 Codex의 모든 권한(계획·작업자 선택·bundle 승인·PLAN 판정)을 행사한다. 대행 중 만든 변경은 반영하되 Codex 복귀 시 재검토 대상으로 표시한다.
