...
> **바뀐 점(2026-09-25)**: 아래 5·6칸의 `rsi adopt`·`rsi rollback`은 이제 파일을 쓰지 않고 항상 거부한다. 같은 컴퓨터 안에서는 "누가 판정했는지"를 이름표만으로 증명할 수 없기 때문이다(B83). 채택은 사람이 GitHub에서 검토·병합하는 커밋으로만 한다. 명령은 판정에 쓸 증거와 바꿀 파일 내용을 보여 주는 용도로 남아 있다.
