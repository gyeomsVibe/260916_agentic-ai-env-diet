...
| 4 관문 | 프로그램 | `rsi gate --candidate 파일` | 채점기 수정, 숫자 직접 기입, 없는 증거, 겹친 표본, 다른 종류 비교, 기준 완화, 한 지표만 개선 |
| 5 판정 | Codex(부재 시 Claude, 또는 사용자) | `rsi adopt --judge codex` | 자기 채점: 판정자 ≠ 작성자 ≠ 검증자 |
| 6 재측정 | 판정자 | `rsi report`의 `trials`, `rsi rollback` | 되돌릴 수 없음: 이전 값을 보관 |
...
8. 다음 10회가 쌓이면 `rsi report`에 재검증 시점이 표시된다. 나빠졌다면 `rsi rollback`으로 되돌린다.
