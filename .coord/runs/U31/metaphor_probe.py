...
from v7_harness.coord.mailbox import Mailbox, MailboxRejected  # noqa: E402
...
def probe_nack_loss() -> None:
    ...
    try:
        ...
        box.nack(claim)
        print("P3 nack after a same-id republish keeps only one of two messages:", verdict(False),
              "(a different content under a claimed id is refused; the original returns to the inbox)")
        return
    box.nack(claim)
...
def probe_bell() -> None:
    ...
    try:
        ...
        sessions.mkdir()
        head = json.dumps({"cwd": str(root), "session_id": "0199aaaa-bbbb-cccc-dddd-eeeeffff0000"})
        (sessions / "s.jsonl").write_text(head + "\n", encoding="utf-8")
        kwargs = {"ring": True, "runner": runner, "sessions_dir": sessions}
    except ImportError:
        pass
    try:
        result = run_sentinel_cycle(root, box, **kwargs)
    except TypeError:
        result = run_sentinel_cycle(root, box)
    wakes = [i for i in box.list_inbox() if i.startswith("wake_")]
    rang = bool(calls) and calls[0][1:3] == ["queue", "--thread"]
    print("P5 P1 wake exists only as a mailbox file (no queue/OS signal):",
          verdict(result["p1_wake_emitted"] and bool(wakes) and not rang), wakes, "| codex queue calls:", len(calls))
