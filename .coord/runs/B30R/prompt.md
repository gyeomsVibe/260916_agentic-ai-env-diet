...
## 현재 원본 상태 반영(중요)
16. 원본 `security.py`에는 이미 `PROTECTED_WATCH_FILES`(settings.json, settings.local.json, config.toml, rules)가 있다. 이를 요구사항 1의 `DEFAULT_WATCH_REINCLUDES`로 통합(하나의 집합)하고, `.claude`·`.codex` 디렉터리 안의 re-include 경로(hooks/agents/skills/commands/AGENTS.md 등)까지 실제로 순회·탐지되게 하라. 현재 구현은 디렉터리 자체가 제외돼 hooks·agents·skills·AGENTS.md 변경을 탐지하지 못한다.
17. 기존 `tests/test_b28_watch_hole.py`는 유지·통과시키고, 필요하면 새 파일로 hooks·agents·skills·AGENTS.md 탐지 테스트를 추가한다. 수정 범위에 `tests/test_b28_watch_hole.py`를 허용한다.
