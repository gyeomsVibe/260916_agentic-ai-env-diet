...
| B75 | P2 | POSIX에서 브로커가 비정상 종료되면 AF_UNIX 소켓 파일이 남아 같은 주소 재기동이 `EADDRINUSE`로 실패(`test_u11_broker` 크래시 재시작, Linux 실측). Windows Named Pipe는 프로세스와 함께 사라져 무관. 해제는 살아 있는 브로커를 가로채지 않도록 접속 시험 후 거부된 경우에만 unlink해야 함(Codex 소유 U11 설계 변경) | 2026-09-25 Claude Linux 회귀 | POSIX(Linux·클라우드 세션)에서 브로커 운용 시 |
| B76 | P2 | 로컬 작업자 pilot의 인수가 새 테스트 1개뿐이면 지시 밖 삭제를 놓친다: P08 bundle이 U15 `coord log` 명령·파서를 통째로 지웠는데 APPLIED(P09에서 복구). U22c와 같은 유형의 재발. 로컬 작업자 과제는 수정 파일을 import하는 기존 테스트 모듈을 인수에 포함하거나 HEAD 대비 삭제 줄 대조를 기본 관문으로 | P08·P09 이력, 2026-09-25 Claude 재검토 | 다음 `--worker local` 과제 작성 시 |
