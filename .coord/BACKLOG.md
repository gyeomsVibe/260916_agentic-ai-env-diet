...
| B72 | RESOLVED (배포 경로 전환은 권한 대기) | Biz항해 세션 재조사(9/22 16:07~17:40, 13지시·도구 217회): 분업 안내 11회 도착에도 olla 0회, 통째 읽기 4건 13.3만 토큰. 원인 (1) 안내는 무시됨 (2) 첫 큰 읽기 때 작업 위치 277자라 읽기 훅이 못 뜸 (3) 편집 중 원본을 훅이 불러 `NameError: re`. 대책: 8천 토큰 넘는 통째 읽기는 Claude Read·Antigravity view_file 모두 거부하고 요약본·줄 범위로 유도, 회귀 통과본만 `D:/AI-Models/olla-release`로 배포(실행기 전환은 공용 도구 변경이라 권한 검사기가 차단). 30초 예산 테스트는 백그라운드 요약 중 1회 초과(재실행 통과) |
