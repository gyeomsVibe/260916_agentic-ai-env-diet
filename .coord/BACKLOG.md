...
| B71 | RESOLVED | 로컬 초안 실측 2건: (1) 빈 입력 파일(인코딩 실패로 0바이트)에 무관한 점검표 12항목을 지어냄 → `olla ask`가 빈 입력을 거부(종료 코드 2) (2) 정상 입력에서도 존재하지 않는 테스트 이름 8/12개를 지어냄 → 명령·파일명이 든 로컬 초안은 실재 여부를 검사한 뒤 쓴다. 점검표는 Claude가 직접 작성(`.coord/codex_return_checklist.md`) |
