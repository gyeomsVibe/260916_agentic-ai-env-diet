...
| B65 | RESOLVED | 출력 규칙 실측 모니터링 완결: Antigravity 트랜스크립트 위치(`<appDataDir>/brain/<id>/.system_generated/logs/transcript.jsonl`) 및 스키마(`USER_INPUT`/`PLANNER_RESPONSE`) 규명 완료(UNKNOWN 해소). `turn_shape` Antigravity 형식 파싱 추가, `_caller()`의 Antigravity 우선 판별 및 `~/.gemini/config/hooks.json` 훅 등록 완료. 실측 15턴 분석 및 단위 테스트 통과, 589개 전체 회귀 exit 0 |
...
| B70 | RESOLVED (실작동 UNKNOWN) | Antigravity에 훅이 없던 공백: 공식 문서(antigravity.google/docs/hooks)대로 `~/.gemini/config/hooks.json`에 `olla hook-agy` 등록(PreInvocation 분업 안내·run_command 깊은 cd 거부·Stop 긴 보고 되돌림). "allow"는 내지 않아 자체 승인 확인을 건너뛰지 않음. 테스트 3건 |
| B71 | RESOLVED | 로컬 초안 실측 2건: (1) 빈 입력 파일(인코딩 실패로 0바이트)에 무관한 점검표 12항목을 지어냄 → `olla ask`가 빈 입력을 거부(종료 코드 2) (2) 정상 입력에서도 존재하지 않는 테스트 이름 8/12개를 지어냄 → 명령·파일명이 든 로컬 초안은 실재 여부를 검사한 뒤 쓴다. 점검표는 Claude가 직접 작성(`.coord/codex_return_checklist.md`) 후속: `olla ask`가 답 속 테스트 모듈·파일 경로의 실재를 검사해 없으면 종료 코드 5(앞선 가짜 초안에서 8/8 적발) |
| B72 | RESOLVED | Biz항해 세션 재조사(9/22 16:07~17:40, 13지시·도구 217회): 분업 안내 11회 도착에도 olla 0회, 통째 읽기 4건 13.3만 토큰. 원인 (1) 안내는 무시됨 (2) 첫 큰 읽기 때 작업 위치 277자라 읽기 훅이 못 뜸 (3) 편집 중 원본을 훅이 불러 `NameError: re`. 대책: 8천 토큰 넘는 통째 읽기는 Claude Read·Antigravity view_file 모두 거부하고 요약본·줄 범위로 유도, 회귀 통과본만 `D:/AI-Models/olla-release`로 배포(실행기 전환은 공용 도구 변경이라 권한 검사기가 차단). 30초 예산 테스트는 백그라운드 요약 중 1회 초과(재실행 통과) | 후속(사용자 허용): 실행기 두 개를 배포본 경로 + `python -P`로 전환, 원본 폴더 안에서도 배포본을 부르는 것 확인 |
| B73 | RESOLVED (효과 측정 대기) | 근본 원인: 에이전트는 도구 목록 안에서 고른다. `olla`는 셸 명령이라 목록에 없어 Read·Grep과 경쟁 못 함(안내 11회에도 0회). 도구 선택은 요청·도구 설명의 의미 일치가 가장 강한 예측 변수(arXiv:2510.00307), 설명만 바꿔도 선택이 바뀜(arXiv:2505.18135). 대책: 의존성 없는 MCP 서버 `v7_harness/olla_mcp.py`(`local_read_map`·`local_draft`·`local_search`)를 Claude(user)·Codex(global)·Antigravity(mcp_config)에 등록, 규칙 v5.21.0을 도구 이름으로 교체(core 5,743→5,355B). 효과는 `olla stats`의 via=mcp 기록으로 측정 |
| B74 | RESOLVED | 평가: olla MCP ↔ SQLite 파일럿 충돌. 데이터 중복 없음(MCP는 읽기·초안만, SQLite에 쓰지 않음; 쓰기 도구 부재를 테스트로 고정). SQLite 전면 편입은 부적합(SQLite는 프로젝트별·쓰기 관문용, MCP는 전 프로젝트 공용이고 도구 목록 노출이 목적). 실제 충돌은 GPU 1개 경합: OLLAMA_NUM_PARALLEL=1이라 요약 작업이 파일럿 로컬 작업자를 줄 세움 → 기계 전역 GPU 임대(파일럿 우선, MCP는 즉시 '사용 중' 반환)와 사용 기록 단일화(파일럿 로컬 실행도 usage.jsonl)가 다음 단계 적용: `adapters/gpu_priority.py` 기계 전역 표식(만료 시각 포함)으로 파일럿 로컬 생성 중 olla CLI(종료 코드 6)·MCP·예열이 즉시 양보, 캐시된 지도는 계속 제공. 파일럿 로컬 실행도 usage.jsonl에 `pilot_local`로 기록(사용 기록 단일화). `olla stats --session`으로 세션 감시, 매시 17분 자동 점검 |
| B75 | P2 (부분 해소: U35 `8a0c11f`로 소켓을 프로젝트 대신 임시 폴더에 둠. 재기동 실패는 남음 → U35-A1 매뉴얼 발행) | POSIX에서 브로커가 비정상 종료되면 AF_UNIX 소켓 파일이 남아 같은 주소 재기동이 `EADDRINUSE`로 실패(`test_u11_broker` 크래시 재시작, Linux 실측). Windows Named Pipe는 프로세스와 함께 사라져 무관. 해제는 살아 있는 브로커를 가로채지 않도록 접속 시험 후 거부된 경우에만 unlink해야 함(Codex 소유 U11 설계 변경) | 2026-09-25 Claude Linux 회귀 | POSIX(Linux·클라우드 세션)에서 브로커 운용 시 |
| B76 | RESOLVED(U34 `d5972cb`, Codex 재검토 대상) — 요청 밖 def/class 삭제 거부, 매뉴얼 검사의 관련 테스트 누락 경고, 허용 범위 강제 | 로컬 작업자 pilot의 인수가 새 테스트 1개뿐이면 지시 밖 삭제를 놓친다: P08 bundle이 U15 `coord log` 명령·파서를 통째로 지웠는데 APPLIED(P09에서 복구). U22c와 같은 유형의 재발. 로컬 작업자 과제는 수정 파일을 import하는 기존 테스트 모듈을 인수에 포함하거나 HEAD 대비 삭제 줄 대조를 기본 관문으로 | P08·P09 이력, 2026-09-25 Claude 재검토 | 다음 `--worker local` 과제 작성 시 |
| B77 | RESOLVED (사용자 승인 2026-09-25; 이 저장소 적용, 전역 설치는 사용자 PC 실행 대기, Windows 실효력 UNVERIFIED) | 활성화 3건: (1) Claude 유료 예약 도구 deny (2) 출석부 자동 훅 (3) 24/7 교환원. 사용자가 이 대화에서 세 건을 승인했다("…승인한다"). 이 저장소 `.claude/settings.json`에 (1)(2) 적용(기본 셸 bash: Linux와 Git Bash용 Windows 공용), 모든 프로젝트는 `python uaos_everywhere/install_uaos_everywhere.py --apply`, (3)은 `--register-sentinel <프로젝트>`(docs/39). Codex Windows 검토(83ef179·67a1539): Linux 제안안의 `cd "$CLAUDE_PROJECT_DIR"`는 PowerShell 셸에서 그대로 돌지 않으므로 PowerShell 훅을 쓰는 설치에는 `tool-configs/claude/settings.uaos-windows-proposed.json`(격리 폴더 exit 0·ACTIVE 확인)을 쓴다. 실제 Claude 로딩·예약 차단·스케줄러 가동은 UNVERIFIED. Codex·Antigravity의 예약 기능 차단 설정은 UNKNOWN(규칙 문단으로만 금지) | U31·U35, docs/36 §6, docs/37 §8, docs/39 §4 | 사용자 PC 설치·새 세션에서 훅 실효력 확인 |
| B78 | P3 | `pilot run --approve`(승인 재생 경로)는 사용량 장부에 행을 남기지 않는다. U35-P1에서 dry-run 행만 기록됨. 승인의 비용은 0에 가깝지만 RSI 분모에서 승인 이벤트가 빠진다 | U35-P1 실측 | 장부로 승인률을 잴 때 |
| B79 | P3 | 매뉴얼의 수치가 발행 뒤 바뀌면 인수가 문자열만 확인해 틀린 값이 반영될 수 있다(U35-P1: "660" 반영 뒤 실제 663). 수치는 인수 명령이 그 자리에서 세게 한다 | U35-P1 실측 | 수치가 든 매뉴얼 작성 시 |
| B80 | P2 | 사용량 장부(`.coord/usage/runs.jsonl`)는 로컬 파일이라 RSI 관문을 거치지 않은 직접 수정을 탐지할 수 없다. 관문은 후보가 장부를 변경 경로로 신고할 때만 거부한다. 행 단위 해시 체인(이전 행 해시 포함)이나 서명으로 변조 탐지 필요 | U36 비판적 검토(docs/38 §3) | RSI 채택이 실제로 쓰이기 시작할 때 |
| B81 | P3 | RSI 비교는 작업자 유형이 같으면 비교 가능으로 본다. 과제 난이도(과제군, task family)가 장부에 없어 쉬운 과제로 '개선'을 보일 수 있다. 파일럿 장부에 매뉴얼 해시·과제군 필드 추가 필요 | U36 비판적 검토(docs/38 §3) | 첫 RSI 후보 판정 전 |
| B82 | P3 | 전역 규칙 3파일은 정본 생성기(`sync-global-rules.ps1`)가 덮어써 설치기의 UAOS 문단이 다음 Apply 때 사라진다. 정본(260718 `shared/global-rules`)에 `uaos_everywhere/uaos_global_rule_block.md`를 넣어야 영구 반영. 이 세션은 해당 저장소 접근 거부 | U37(docs/39 §2) | 사용자 PC에서 정본 갱신 시 |
| B83 | RESOLVED(3cc40db) | rsi adopt/rollback fail closed (UNAUTHENTICATED_ACTOR) — 같은 계정 이름표는 인증이 아님, 자동 채택 차단 | 2026-09-25 Claude 비판적 재검토(08장) | — |
| B84 | P4 | 전역 설치 후 이 저장소에서는 프로젝트 `.claude/settings.json` 훅과 전역 훅이 둘 다 실행돼 출석 기록 2회·세션 시작 요약 줄 2회(수십 토큰). 기록 내용은 같아 무해 | 2026-09-25 Claude 비판적 재검토(08장) | 사용자 PC 전역 설치 후 |
| B85 | RESOLVED(1d9bcc0) | 유료 작업자(agy·claude) 및 cascade 승격 전체에 예산 관문(remote_budget_tokens) 강제 — 예산 초과·미보고 시 BLOCKED | 2026-09-25 U38 실측 | — |
