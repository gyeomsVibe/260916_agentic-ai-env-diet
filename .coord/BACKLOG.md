...
| B77 | 승인 범위 확인 대기; Windows 제안안은 비활성 검증 완료 | 활성화 3건: (1) Claude 유료 예약 도구 deny(`CronCreate`·`ScheduleWakeup`·원격 트리거) (2) 출석부 자동 훅 (3) 24/7 교환원 Windows 작업 스케줄러 등록. Linux 제안안 `tool-configs/claude/settings.uaos-proposed.json`을 Windows에 그대로 쓸 수 없어 `settings.uaos-windows-proposed.json`을 별도 발행했다. PowerShell SessionStart 명령은 격리 폴더에서 exit 0·ACTIVE; 실제 Claude 로딩·예약 차단·스케줄러 가동은 UNVERIFIED. Codex·Antigravity의 예약 기능 차단 방법도 UNKNOWN | U31·U35, docs/36 §6·docs/37 §8 | 세 설정별 사용자 승인·활성 효력 확인 |
