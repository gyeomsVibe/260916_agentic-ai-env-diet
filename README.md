...
| 믿을 근거 | 인수 테스트 **422개 통과**(1 skip), 모든 수치는 실행 기록과 함께 저장소에 있음 |
...
| **로컬 모델(Ollama)** | 세 도구가 어느 프로젝트에서든 부려 쓰는 손. 기계적인 일은 먼저 `olla`로, 판단이 필요할 때만 자기 한도를 씀 ([작동 원리](docs/ollama/README.md)) | 상주 인력 |
...
| 항목 | 상태 |
|---|---|
| 파일럿·제어층·조율 스트림 | 구현·검증 완료(422 테스트 통과) |
